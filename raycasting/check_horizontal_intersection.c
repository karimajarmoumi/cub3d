...
/*   Updated: 2023/09/12 22:11:12 by kjarmoum         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_coord	get_f_horizontal_intersection(t_map map, float angle, float *dx,
		float *dy)
{
	t_coord	coord;
	...
	if (is_up(angle) == 1)
		coord.x = map.player_pos.x - fabs(map.player_pos.y - coord.y)
			/ tan(angle);
	else
		coord.x = map.player_pos.x + fabs(map.player_pos.y - coord.y)
			/ tan(angle);
